    ...
    public function create(array $input, string $actor): array
    {
        ...
        $this->db->transaction(function () use ($id, $title, $reward, $questions, $actor, $now, $input) {
            ...
            foreach ($questions as $i => $q) {
                $options = $q['options'] ?? [];
                if (!is_array($options) || count($options) < 2) throw new BillingError('В каждом вопросе минимум 2 варианта.');
                $this->db->execute('INSERT INTO poll_questions(id,poll_id,text,options,"order") VALUES(?,?,?,?,?)', [Database::id(), $id, (string)($q['text'] ?? ''), json_encode(array_values($options), JSON_THROW_ON_ERROR), $i]);
            }
    ...
    public function submit(string $userId, string $pollId, array $answers): array
    {
        return $this->db->transaction(function () use ($userId, $pollId, $answers) {
            ...
            foreach ($questions as $i => $q) {
                ...
                if ($answer === null || $answer === '') throw new BillingError('Ответьте на все вопросы.');
                $normalized[] = $answer;
            }
            $reward = 0;
