...
final class AuraPayProvider extends AbstractProvider
{
    public function id(): string { return 'aurapay'; }
    public function name(): string { return 'AuraPay'; }
    public function configured(): bool
    {
        return ($this->config['AURAPAY_ENABLED'] ?? '0') === '1'
            && ($this->config['AURAPAY_API_KEY'] ?? '') !== ''
            && ($this->config['AURAPAY_SHOP_ID'] ?? '') !== ''
            && ($this->config['AURAPAY_SECRET_KEY'] ?? '') !== '';
    }
    private function api(string $endpoint, array $data = []): array
    {
        $key = (string)($this->config['AURAPAY_API_KEY'] ?? '');
        $shopId = (string)($this->config['AURAPAY_SHOP_ID'] ?? '');
        if ($key === '' || $shopId === '') throw new BillingError('AuraPay не настроен.');
        $resp = $this->json('POST', 'https://api.aurapay.tech/'.$endpoint, [
            'headers' => ['X-ApiKey' => $key, 'X-ShopId' => $shopId, 'Content-Type' => 'application/json'],
            'json' => $data,
        ]);
        if (($resp['status'] ?? '') !== 'success' && ($resp['success'] ?? false) !== true) throw new BillingError('AuraPay: '.($resp['message'] ?? 'API error'));
    ...
    public function createTopup(array $topup, array $user): array
    {
        ...
        if (!$url || !str_starts_with($url, 'https://')) throw new BillingError('AuraPay не вернул ссылку оплаты.');
    ...
    {
        $secret = (string)($this->config['AURAPAY_SECRET_KEY'] ?? '');
