...
final class EtoplatezhiProvider extends AbstractProvider
{
    public function id(): string { return 'etoplatezhi'; }
    public function name(): string { return 'Etoplatezhi'; }
    public function configured(): bool
    {
        return ($this->config['ETOPLATEZHI_ENABLED'] ?? '0') === '1'
            && ($this->config['ETOPLATEZHI_PROJECT_ID'] ?? '') !== ''
            && ($this->config['ETOPLATEZHI_SECRET_KEY'] ?? '') !== '';
    }
    private function api(string $endpoint, array $data = []): array
    {
        $projectId = (string)($this->config['ETOPLATEZHI_PROJECT_ID'] ?? '');
        $secret = (string)($this->config['ETOPLATEZHI_SECRET_KEY'] ?? '');
        if ($projectId === '' || $secret === '') throw new BillingError('Etoplatezhi не настроен.');
        ...
        $resp = $this->json('POST', 'https://api.etoplatezhi.ru/'.$endpoint, ['json' => $data]);
        if (($resp['status'] ?? '') !== 'success' && ($resp['success'] ?? false) !== true) throw new BillingError('Etoplatezhi: '.($resp['message'] ?? 'API error'));
    ...
    public function createTopup(array $topup, array $user): array
    {
        ...
        if (!$url || !str_starts_with($url, 'https://')) throw new BillingError('Etoplatezhi не вернул ссылку оплаты.');
