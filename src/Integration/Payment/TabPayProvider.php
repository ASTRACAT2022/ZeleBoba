...
final class TabPayProvider extends AbstractProvider
{
    public function id(): string { return 'tabpay'; }
    public function name(): string { return 'TabPay'; }
    public function configured(): bool
    {
        return ($this->config['TABPAY_ENABLED'] ?? '0') === '1'
            && ($this->config['TABPAY_API_KEY'] ?? '') !== ''
            && ($this->config['TABPAY_WEBHOOK_SECRET'] ?? '') !== '';
    }
    private function baseUrl(): string
    {
        return rtrim($this->config['TABPAY_BASE_URL'] ?? 'https://tabpay.org/api', '/');
    }
    private function api(string $endpoint, array $data = []): array
    {
        $key = (string)($this->config['TABPAY_API_KEY'] ?? '');
        if ($key === '') throw new BillingError('TabPay не настроен.');
        $resp = $this->json('POST', $this->baseUrl().'/'.$endpoint, [
            'headers' => ['X-Api-Key' => $key, 'Content-Type' => 'application/json'],
            'json' => $data,
        ]);
        if (($resp['status'] ?? '') !== 'success' && ($resp['success'] ?? false) !== true) throw new BillingError('TabPay: '.($resp['message'] ?? 'API error'));
    ...
    public function createTopup(array $topup, array $user): array
    {
        ...
        if (!$url || !str_starts_with($url, 'https://')) throw new BillingError('TabPay не вернул ссылку оплаты.');
    ...
    {
        $secret = (string)($this->config['TABPAY_WEBHOOK_SECRET'] ?? '');
