...
final class JupiterProvider extends AbstractProvider
{
    public function id(): string { return 'jupiter'; }
    public function name(): string { return 'Jupiter'; }
    public function configured(): bool
    {
        return ($this->config['JUPITER_ENABLED'] ?? '0') === '1'
            && ($this->config['JUPITER_TOKEN'] ?? '') !== ''
            && ($this->config['JUPITER_SECRET'] ?? '') !== '';
    }
    private function baseUrl(): string
    {
        return rtrim($this->config['JUPITER_BASE_URL'] ?? 'https://app.juppiter.tech', '/');
    }
    private function api(string $endpoint, array $data = []): array
    {
        $token = (string)($this->config['JUPITER_TOKEN'] ?? '');
        $secret = (string)($this->config['JUPITER_SECRET'] ?? '');
        if ($token === '' || $secret === '') throw new BillingError('Jupiter не настроен.');
        $data['token'] = $token;
        ...
        if (($resp['status'] ?? '') !== 'success' && ($resp['success'] ?? false) !== true) throw new BillingError('Jupiter: '.($resp['message'] ?? 'API error'));
    ...
    public function createTopup(array $topup, array $user): array
    {
        $result = $this->api('payment/create', [
            ...
            'method_id' => (string)($this->config['JUPITER_METHOD_ID'] ?? ''),
            'return_url' => rtrim($this->config['APP_URL'] ?? '', '/').'/balance',
        ]);
        $url = $result['url'] ?? $result['payment_url'] ?? null;
        if (!$url || !str_starts_with($url, 'https://')) throw new BillingError('Jupiter не вернул ссылку оплаты.');
