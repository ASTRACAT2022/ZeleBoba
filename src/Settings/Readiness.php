...
final class Readiness
{
    public static function report(Container $app):array
    {
        ...
        $add('Реальные адаптеры',in_array($c['PAYMENT_DRIVER'],['yookassa','freekassa'],true)&&$c['PROVISION_DRIVER']==='remnawave','Демо не выдаёт реальный доступ.');
        $add('Параметры оплаты и панели',Settings::purchaseErrors($c)===[],'Магазин, ключи и группа панели должны быть заполнены.');
        $paymentCheck=$c['PAYMENT_DRIVER']==='freekassa'?'freekassa':'yookassa';
