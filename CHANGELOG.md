...
### Добавлено
- **Объединение подписок (клиентский self-service)** — `SubscriptionMergeService::merge()`: пользователь переносит все ресурсы с одной подписки на другую (остаток дней, лимит и докупка трафика, устройства, использованный трафик) и исходная подписка **физически удаляется**. Всё в одной транзакции под row-локами; детерминированный порядок локаций исключает deadlock; полный аудит + timeline + Ops-операция; после коммита идемпотентная синхронизация панели. UI на главной странице (панель «Объединение подписок» при ≥2 активных). Проверено `tests/merge_faulttest.php`.
- **Миграция `035_merge_hard_delete`**: FK на `subscriptions` пересозданы так, чтобы физическое удаление подписки было безопасным — runtime-провижининг (`provisioning_accounts`/`provisioning_operations`) каскадно удаляется (CASCADE), финансовые записи (`operations`/`order_items`/`orders`/`operation_events`) сохраняются со ссылкой, обнулённой до NULL (SET NULL) — аудит платежей никогда не теряется.
...
### Исправлено
- **Оплаченные покупки/продления больше не умирают на `payment.verify`** — `subscriptions.traffic_used_gb` в live-схеме потерял `DEFAULT` (бил NOT NULL без default), из-за чего `BillingService::settle()` создавал новую подписку без этой колонки и падал с `SQLSTATE 23502` → 8 ретраев → dead-letter (21 задание). Миграция `036` восстанавливает `DEFAULT 0`, а код теперь передаёт `traffic_used_gb=0` явно — сеттлмент не зависит от дефолта схемы.
- **Ежедневное автосписание (тариф «Сутки») работает надёжно даже в долгоживущем воркере** — воркер замораживает `Container`-конфиг при старте, поэтому снапшот `AUTORENEW_ENABLED` протухал, и daily auto-enable молча пропускался для покупок, сеттленных из воркера (кошелёк/автопокупка): подписка оставалась с `auto_renew=0`, никогда не списывалась и истекала. Новая `autoRenewEnabled()` читает `AUTORENEW_ENABLED` из `app_settings` прямо в момент решения (как уже делал `Worker::renew`) и используется и в `settle()`, и в `setAutoRenew(true)`.
