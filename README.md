...
### Биллинг VPN-подписок для Remnawave

**Принимайте оплату, выдавайте доступ и управляйте подписками в одном месте.**
...
**ZeleBoba** — самостоятельная платформа для продажи VPN-подписок через [Remnawave](https://github.com/remnawave). Клиентский кабинет, Telegram-бот и админка работают с общей историей заказов, платежей и выдачи доступа. Платежи принимает Platega; кошелёк, продления, подарки и реферальная программа помогают управлять подписками в одном месте.

Финансовые операции опираются на транзакции PostgreSQL, ключи идемпотентности и надёжную очередь задач. Это помогает избежать повторного зачисления и восстановить обработку после сбоя.

Рабочий веб-интерфейс сейчас обслуживает PHP. Перенос серверной логики на Rails идёт поэтапно и выключен для боевого трафика; [статус миграции](docs/rails-migration-status.md) описывает готовые части и оставшиеся проверки.
