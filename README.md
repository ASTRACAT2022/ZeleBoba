# ZeleBoba Billing · 0.4 — FreeKassa + Telegram-дублер

PHP-биллинг подписок для Remnawave: веб-кабинет, Telegram-бот (дублер кабинета через зеркало), FreeKassa (API) / ЮKassa и веб-админка. Подготовлен и проверен контейнерный кандидат для развёртывания. **Реальный приём денег включается только после проверки конкретного магазина, панели и публичного HTTPS-домена.**
