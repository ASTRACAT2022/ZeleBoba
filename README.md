# ZeleBoba Billing · 1.0 — полный функциональный перенос Bedolaga

PHP-биллинг подписок для Remnawave: веб-кабинет, Telegram-бот, кошелёк, 26 платёжных провайдеров, промокоды, реферальная программа, подарки, триалы, маркетинг и полная админка. **Реальный приём денег включается только после проверки конкретного магазина, панели и публичного HTTPS-домена.**
...
- Единый кошелёк: пополнение любым из 26 провайдеров, покупка с баланса, автопокупка после пополнения (умная корзина).
- Платёжные провайдеры: ЮKassa, FreeKassa, CryptoBot, Telegram Stars, Lava, WATA, Heleket, Platega, Tribute, MulenPay, Pal24, CloudPayments, Kassa AI, RioPay, SeverPay, PayPear, RollyPay, Overpay, AuraPay, Etoplatezhi, Antilopay, Jupiter, Donut, CisPay, TabPay, ParityPay.
...
Ориентир по возможностям — [Bedolaga](https://github.com/BEDOLAGA-DEV/remnawave-bedolaga-telegram-bot). Исходный код и ресурсы не переносились; это независимая PHP-реализация с полным функциональным паритетом по ядру (кошелёк, провайдеры, промокоды, рефералы, подарки, триалы, маркетинг, админка).
