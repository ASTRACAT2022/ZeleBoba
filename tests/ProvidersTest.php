...
final class ProvidersTest extends TestCase
{
    ...
    public function testCryptoBotTopupCheckoutAndVerify():void
    {
        ...
        $http=new MockHttpClient(function($method,$url,$options)use(&$calls){
            ...
            if (($body['currency_type']??'')==='crypto') return new MockResponse(json_encode(['ok'=>true,'result'=>['invoice_id'=>42,'bot_invoice_url'=>'https://t.me/bot/invoice']]));
            return new MockResponse(json_encode(['ok'=>true,'result'=>['items'=>[['invoice_id'=>42,'status'=>'paid','amount'=>'199.00','payload'=>'topup:abc']]]]));
        });
    ...
    public function testYooKassaOrderCheckoutAndVerify():void
    {
        ...
        $http=new MockHttpClient(function($method,$url,$options)use(&$calls){
            ...
            return new MockResponse(json_encode(['id'=>'pay-1','status'=>'succeeded','paid'=>true,'test'=>false,'metadata'=>['order_id'=>'o1'],'amount'=>['value'=>'199.00','currency'=>'RUB']]));
        });
        $r=$this->registry($config,$http);
        $svc=new PaymentService($this->db,$this->billing,$http,$config,$r);
    ...
    public function testStarsProviderConfiguredCheck():void
    {
        ...
        self::assertArrayHasKey('telegram_stars',$r->enabled());
    }
    ...
    public function testAllProvidersRegistered():void
    {
        ...
        $r=$this->registry($config);
        $enabled=$r->enabled();
        self::assertCount(26,$enabled);
        foreach (['yookassa','freekassa','cryptobot','telegram_stars','lava','wata','heleket','platega','tribute','mulenpay','pal24','cloudpayments','kassa_ai','riopay','severpay','paypear','rollypay','overpay','aurapay','etoplatezhi','antilopay','jupiter','donut','cispay','tabpay','paritypay'] as $id) {
