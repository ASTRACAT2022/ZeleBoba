...
final class CutoverCheck extends Command
{
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $errors[] = 'database is not reachable';
        }
        if ($this->option('production')) {
            ...
            if (config('payments.driver') === 'demo') {
                $errors[] = 'PAYMENT_DRIVER=demo is forbidden';
            }
            if (config('queue.default') === 'sync') {
                $errors[] = 'QUEUE_CONNECTION=sync is forbidden';
            }
            if (config('queue.default') !== 'redis') {
                $errors[] = 'QUEUE_CONNECTION=redis is required for the production topology';
            }
            if (! class_exists(Horizon::class)) {
                $errors[] = 'Laravel Horizon is not installed';
            }
            if (config('payments.driver') === 'yookassa' && (! (string) config('services.yookassa.shop_id') || ! (string) config('services.yookassa.secret'))) {
                $errors[] = 'YooKassa credentials are missing';
            }
            if (config('payments.driver') === 'freekassa' && (! (string) config('services.freekassa.shop_id') || ! (string) config('services.freekassa.api_key') || ! (string) config('services.freekassa.secret2'))) {
                $errors[] = 'FreeKassa credentials are missing';
            }
