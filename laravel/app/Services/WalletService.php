...
final class WalletService
{
    ...
    {
        if ($amount < 1) throw ValidationException::withMessages(['amount'=>'Сумма должна быть положительной.']);
        DB::transaction(function () use ($userId, $amount, $type, $description, $externalId): void {
            ...
            if (!$user || (int) $user->balance_kopeks < $amount) throw ValidationException::withMessages(['balance'=>'Недостаточно средств на балансе.']);
            $now = time();
    ...
    {
        if ($amount < 100 || $amount > 100_000_000 || !preg_match('/^[a-zA-Z0-9:_-]{8,128}$/D', $key)) throw ValidationException::withMessages(['amount'=>'Сумма пополнения: от 1 до 1 000 000 ₽.']);
        return DB::transaction(function () use ($userId, $amount, $key): object {
            ...
            if ($existing) {
                if ((int) $existing->amount_kopeks !== $amount || $existing->provider !== config('payments.driver')) throw ValidationException::withMessages(['idempotency_key'=>'Этот ключ уже использован для другой суммы или способа оплаты.']);
                return $existing;
            }
    ...
    {
        if (app()->isProduction() || config('payments.driver') !== 'demo') abort(404);
        DB::transaction(function () use ($topupId,$userId): void {
            $topup=DB::table('topups')->where('id',$topupId)->lockForUpdate()->first();
            if (!$topup || $topup->user_id !== $userId || $topup->status !== 'pending') { if ($topup && $topup->status==='paid') return; abort(404); }
    ...
    {
        if ($paymentId === '' || strlen($paymentId) > 100) throw ValidationException::withMessages(['payment'=>'Некорректный идентификатор платежа.']);
        DB::transaction(function () use ($topupId, $provider, $paymentId, $amount, $currency): void {
            $topup = DB::table('topups')->where('id', $topupId)->lockForUpdate()->first();
            if (!$topup || $topup->provider !== $provider || (int) $topup->amount_kopeks !== $amount || $topup->currency !== $currency) throw ValidationException::withMessages(['payment'=>'Платёж не соответствует пополнению.']);
            if (DB::table('payment_receipts')->where('provider', $provider)->where('payment_id', $paymentId)->exists()) throw ValidationException::withMessages(['payment'=>'Платёж уже использован для заказа.']);
            $duplicate = DB::table('topups')->where('provider_payment_id', $paymentId)->first();
            if ($duplicate && $duplicate->id !== $topupId) throw ValidationException::withMessages(['payment'=>'Платёж уже использован для другого пополнения.']);
            if ($topup->status === 'paid' && $topup->provider_payment_id === $paymentId) return;
            if ($topup->status !== 'pending') throw ValidationException::withMessages(['topup'=>'Пополнение уже обработано.']);
    ...
    private function settleLocked(object $topup, string $paymentId): void
    {
        ...
        if (!$user) throw ValidationException::withMessages(['user'=>'Аккаунт не найден.']);
        ...
        if (DB::getSchemaBuilder()->hasTable('customer_timeline')) DB::table('customer_timeline')->insert(['id'=>bin2hex(random_bytes(16)), 'user_id'=>$topup->user_id, 'event_type'=>'balance.topped_up', 'payload'=>json_encode(['topup_id'=>$topup->id, 'amount_kopeks'=>(int) $topup->amount_kopeks], JSON_THROW_ON_ERROR), 'occurred_at'=>$now, 'recorded_at'=>(int) floor(microtime(true) * 1_000_000)]);
    }
}
