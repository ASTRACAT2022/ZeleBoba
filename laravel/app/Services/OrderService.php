...
final class OrderService
{
    ...
    {
        if (!preg_match('/^[a-zA-Z0-9:_-]{8,128}$/D', $key)) throw ValidationException::withMessages(['idempotency_key' => 'Некорректный ключ операции.']);
        ...
        return DB::transaction(function () use ($userId, $planId, $key): object {
            ...
            if ($existing) {
                if ($existing->plan_id !== $planId) throw ValidationException::withMessages(['idempotency_key' => 'Этот ключ уже использован для другого тарифа.']);
                return $existing;
            }
            $plan = DB::table('plans')->where('id', $planId)->where('active', 1)->first();
            if (!$plan) throw ValidationException::withMessages(['plan_id' => 'Тариф недоступен.']);
    ...
    public function settleVerified(string $orderId, string $provider, string $paymentId, int $amount, string $currency): void
    {
        DB::transaction(function () use ($orderId, $provider, $paymentId, $amount, $currency): void {
            ...
            if (!$order || $order->provider !== $provider || (int) $order->price_minor !== $amount || $order->currency !== $currency) throw ValidationException::withMessages(['payment' => 'Платёж не соответствует заказу.']);
            $receipt = DB::table('payment_receipts')->where('provider', $provider)->where('payment_id', $paymentId)->first();
            if ($receipt) return;
            if ($order->status !== 'pending') throw ValidationException::withMessages(['order' => 'Заказ уже обработан.']);
