...
final class SubscriptionLifecycleService
{
    ...
    public function startTrial(string $userId, string $planId): string
    {
        return DB::transaction(function () use ($userId, $planId): string {
            ...
            $alreadyUsed = DB::table('subscriptions')->where('user_id', $userId)->where(function ($q): void { $q->where('is_trial', 1)->orWhereIn('status', ['active', 'trial', 'limited', 'provisioning']); })->exists();
            if (!$user || (int) ($user->disabled ?? 0) === 1 || (int) ($user->has_had_paid_subscription ?? 0) === 1 || $alreadyUsed || !$plan) throw ValidationException::withMessages(['trial'=>'Триал недоступен.']);
            $days = max(1, (int) ($plan->trial_duration_days ?? 3)); $now = time(); $id = bin2hex(random_bytes(16));
    ...
    public function setAutoRenew(string $userId, string $subscriptionId, bool $enabled): void
    {
        DB::transaction(function () use ($userId, $subscriptionId, $enabled): void {
            ...
            if (!$subscription || (int) ($subscription->is_trial ?? 0) === 1) throw ValidationException::withMessages(['subscription'=>'Подписка не поддерживает автопродление.']);
            $plan = DB::table('plans')->where('id', $subscription->plan_id)->first();
            if (!$plan) throw ValidationException::withMessages(['subscription'=>'Тариф не найден.']);
    ...
    public function processDue(): array
    {
        ...
        foreach (DB::table('subscriptions')->where('auto_renew', 1)->whereNotNull('renew_at')->where('renew_at', '<=', $now)->whereIn('status', ['active', 'limited'])->limit(100)->pluck('id') as $id) {
            try { $this->renew((string) $id, $now); $renewed++; } catch (ValidationException) { $failed++; }
        }
        $expired = 0;
        foreach (DB::table('subscriptions')->where('status', 'active')->where('expires_at', '<=', $now)->limit(100)->pluck('id') as $id) {
            $expired += DB::transaction(function () use ($id, $now): int {
                ...
                if (!$subscription || $subscription->status !== 'active' || (int) $subscription->expires_at > $now) return 0;
    ...
    private function renew(string $subscriptionId, int $now): void
    {
        DB::transaction(function () use ($subscriptionId, $now): void {
            ...
            if (!$sub || !(int) $sub->auto_renew || (int) $sub->renew_at > $now) return;
            $plan = DB::table('plans')->where('id', $sub->renew_plan_id ?: $sub->plan_id)->where('active', 1)->first();
            if (!$plan) throw ValidationException::withMessages(['renew'=>'Тариф для продления недоступен.']);
            $price = (int) ($sub->renew_price_minor ?: $plan->price_minor);
            try { $this->wallet->debit($sub->user_id, $price, 'subscription_renewal', 'Автопродление: '.$plan->name, $sub->id.':'.((int) $sub->expires_at)); }
            catch (ValidationException $e) { DB::table('subscriptions')->where('id', $sub->id)->update(['renew_failed_at'=>$now, 'renew_fail_count'=>(int) $sub->renew_fail_count + 1, 'renew_at'=>$now + 86400, 'updated_at'=>$now]); throw $e; }
            $base = max($now, (int) $sub->expires_at); $expiry = $base + (int) $plan->duration_days * 86400;
