...
final class CheckoutService
{
    public function create(string $orderId): object
    {
        ...
        if (! $order || $order->status !== 'pending' || $order->checkout_url) {
            return $order;
        }
        ...
        if ($order->provider === 'freekassa') {
            return $this->createFreeKassa($order);
        }
        if ($order->provider !== 'yookassa') {
            throw ValidationException::withMessages(['payment' => 'Этот платёжный провайдер ещё не перенесён в Laravel.']);
        }

        $shop = (string) config('services.yookassa.shop_id');
        $secret = (string) config('services.yookassa.secret');
        if ($shop === '' || $secret === '') {
            throw ValidationException::withMessages(['payment' => 'ЮKassa не настроена.']);
        }
        ...
        if (! $response->successful() || ! is_string($paymentId) || ! is_string($url) || ! str_starts_with($url, 'https://')) {
            throw ValidationException::withMessages(['payment' => 'Провайдер не вернул ссылку оплаты.']);
        }
    ...
    public function createTopup(string $topupId): object
    {
        ...
        if (! $topup || $topup->status !== 'pending' || $topup->checkout_url) {
            return $topup;
        }
        ...
        if ($topup->provider === 'freekassa') {
            return $this->createFreeKassa($topup, 'topups');
        }
        if ($topup->provider !== 'yookassa') {
            throw ValidationException::withMessages(['payment' => 'Этот платёжный провайдер ещё не перенесён в Laravel.']);
        }
        $shop = (string) config('services.yookassa.shop_id');
        $secret = (string) config('services.yookassa.secret');
    ...
    private function createFreeKassa(object $order, string $table = 'orders'): object
    {
        ...
        if ($shop === '' || $key === '') {
            throw ValidationException::withMessages(['payment' => 'FreeKassa не настроена.']);
        }
        if (time() - (int) $order->created_at > 23 * 3600) {
            throw ValidationException::withMessages(['payment' => 'Требуется ручная сверка платежа.']);
        }
        $user = DB::table('users')->where('id', $order->user_id)->first();
        $email = $user->email ?? null;
        if (! is_string($email) || ! filter_var($email, FILTER_VALIDATE_EMAIL)) {
            throw ValidationException::withMessages(['payment' => 'Некорректный email для FreeKassa.']);
        }
        ...
        if (! $response->successful() || ($data['type'] ?? '') === 'error' || ! is_scalar($id) || ! is_string($url) || ! str_starts_with($url, 'https://')) {
            throw ValidationException::withMessages(['payment' => 'FreeKassa не вернула ссылку оплаты.']);
        }
    ...
    private function nonce(): int
    {
        ...
        static $last = 0;
        $next = (int) (microtime(true) * 1_000_000) + 8_500_000_000_000_000_000;
