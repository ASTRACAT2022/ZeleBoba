...
final class VerifiedTopupTest extends TestCase
{
    ...
    {
        parent::setUp();
        foreach (['customer_timeline', 'transactions', 'topups', 'payment_receipts', 'orders', 'sessions', 'users'] as $table) {
        ...
        Schema::create('users', function (Blueprint $t): void {
            ...
            $t->bigInteger('balance_kopeks')->default(0);
            $t->integer('has_made_first_topup')->default(0);
        });
        ...
        Schema::create('topups', function (Blueprint $t): void {
            $t->string('id')->primary();
            $t->string('user_id');
            $t->integer('amount_kopeks');
            $t->string('currency');
            $t->string('status');
            $t->string('provider');
            $t->string('provider_payment_id')->nullable()->unique();
            $t->text('checkout_url')->nullable();
            $t->string('idempotency_key');
            $t->integer('referral_first')->default(0);
            $t->integer('created_at');
            $t->integer('paid_at')->nullable();
