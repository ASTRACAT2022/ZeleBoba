...
final class LegacyAuthTest extends TestCase
{
    ...
    {
        parent::setUp();
        foreach (['subscriptions', 'orders', 'plans', 'sessions', 'users'] as $table) Schema::dropIfExists($table);
        Schema::create('users', function (Blueprint $table): void {
            $table->string('id')->primary(); $table->string('email')->nullable()->unique(); $table->string('password_hash')->nullable();
            $table->string('telegram_id')->nullable(); $table->string('role')->default('customer'); $table->integer('disabled')->default(0); $table->integer('promo_offer_discount_percent')->default(0); $table->unsignedBigInteger('promo_offer_discount_expires_at')->nullable(); $table->unsignedBigInteger('created_at');
        });
        Schema::create('sessions', function (Blueprint $table): void {
            $table->string('id')->primary(); $table->string('user_id'); $table->string('csrf'); $table->unsignedBigInteger('expires_at'); $table->unsignedBigInteger('admin_verified_until')->default(0);
        });
        Schema::create('plans', function (Blueprint $table): void { $table->string('id')->primary(); $table->string('name'); $table->integer('price_minor')->default(0); $table->integer('duration_days')->default(30); $table->unsignedBigInteger('traffic_bytes')->default(0); $table->integer('devices')->default(1); $table->integer('active')->default(1); });
        Schema::create('orders', function (Blueprint $table): void { $table->string('id')->primary(); $table->string('user_id'); $table->string('plan_name'); $table->integer('price_minor'); $table->string('status'); $table->unsignedBigInteger('created_at'); });
        Schema::create('subscriptions', function (Blueprint $table): void { $table->string('id')->primary(); $table->string('user_id'); $table->string('order_id')->nullable(); $table->string('plan_id')->nullable(); $table->string('status'); $table->unsignedBigInteger('expires_at'); $table->unsignedBigInteger('created_at'); });
    }
    ...
    {
        $id = str_repeat('4', 32); $password = 'correct-horse-battery'; $salt = 'legacy-salt';
    ...
    {
        $id = str_repeat('5', 32); $token = str_repeat('b', 64);
