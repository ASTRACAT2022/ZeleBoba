...
final class DemoCheckoutTest extends TestCase
{
    ...
    {
        parent::setUp();
        foreach (['customer_timeline','subscriptions','ledger_entries','payment_receipts','orders','plans','sessions','users'] as $table) Schema::dropIfExists($table);
        Schema::create('users', function (Blueprint $t): void { $t->string('id')->primary(); $t->string('email'); $t->string('telegram_id')->nullable(); $t->string('role')->default('customer'); $t->integer('disabled')->default(0); $t->integer('promo_offer_discount_percent')->default(0); $t->unsignedBigInteger('promo_offer_discount_expires_at')->nullable(); });
        ...
        Schema::create('plans', function (Blueprint $t): void { $t->string('id')->primary(); $t->string('name'); $t->integer('price_minor'); $t->string('currency'); $t->integer('duration_days'); $t->unsignedBigInteger('traffic_bytes'); $t->integer('devices'); $t->integer('active'); });
        Schema::create('orders', function (Blueprint $t): void { $t->string('id')->primary(); $t->string('user_id'); $t->string('plan_id'); $t->string('idempotency_key'); $t->integer('price_minor'); $t->string('currency'); $t->string('plan_name'); $t->integer('duration_days'); $t->unsignedBigInteger('traffic_bytes'); $t->integer('devices'); $t->string('status'); $t->string('provider'); $t->string('provider_payment_id')->nullable(); $t->text('checkout_url')->nullable(); $t->unsignedBigInteger('created_at'); $t->unsignedBigInteger('paid_at')->nullable(); $t->unique(['user_id','idempotency_key']); });
        Schema::create('payment_receipts', function (Blueprint $t): void { $t->string('provider'); $t->string('payment_id'); $t->string('order_id'); $t->integer('amount_minor'); $t->string('currency'); $t->unsignedBigInteger('created_at'); $t->primary(['provider','payment_id']); });
        Schema::create('ledger_entries', function (Blueprint $t): void { $t->string('id')->primary(); $t->string('order_id'); $t->string('account'); $t->integer('amount_minor'); $t->string('currency'); $t->unsignedBigInteger('created_at'); });
        Schema::create('subscriptions', function (Blueprint $t): void { $t->string('id')->primary(); $t->string('order_id'); $t->string('user_id'); $t->string('status'); $t->unsignedBigInteger('expires_at'); $t->unsignedBigInteger('created_at'); $t->string('plan_id'); $t->integer('traffic_limit_gb'); $t->integer('device_limit'); $t->string('remote_id')->nullable(); $t->text('subscription_url')->nullable(); });
        Schema::create('customer_timeline', function (Blueprint $t): void { $t->string('id')->primary(); $t->string('user_id'); $t->string('event_type'); $t->text('payload'); $t->unsignedBigInteger('occurred_at'); $t->unsignedBigInteger('recorded_at'); });
    }

    public function test_demo_payment_is_idempotent_and_provisions_the_subscription(): void
    {
        $user = str_repeat('6', 32); $token = str_repeat('c', 64);
    ...
    public function test_yookassa_webhook_is_verified_by_yookassa_before_settlement(): void
    {
        ...
        $user = str_repeat('7', 32); $orderId = str_repeat('8', 32);
    ...
    public function test_yookassa_checkout_is_created_with_an_idempotency_key(): void
    {
        ...
        $user = str_repeat('9', 32); $token = str_repeat('d', 64);
    ...
    public function test_freekassa_checkout_is_signed_and_persisted(): void
    {
        ...
        $user = str_repeat('a', 32); $token = str_repeat('e', 64);
    ...
    public function test_freekassa_webhook_signature_and_api_verification_are_required(): void
    {
        ...
        $user = str_repeat('b', 32); $orderId = str_repeat('c', 32);
        ...
        $amount = '199.00'; $sign = md5('123:'.$amount.':secret-word:'.$orderId);
