...
| Синхронизация | `remnawave:sync` + кнопка в админке + автораз в час: сверка сроков/трафика/устройств, пересоздание пропавших, отключение истёкших | Полное зеркалирование внешних юзеров панели |
...
+
+## Синхронизация Remnawave ↔ ZeleBoba
+
+- `remnawave:sync` (CLI) или кнопка «Синхронизировать с Remnawave» в админке, автораз в час через scheduler.
+- Что делает: для активных подписок сверяет `expireAt`/`trafficLimitBytes`/`hwidDeviceLimit` с панелью и чинит дрейф через `PATCH /api/users`; если юзер пропал на панели — пересоздаёт (`POST /api/users`, username детерминированный `zb_<subscription_id>`); истёкшие подписки — `status=DISABLED` на панели.
+- `--dry-run` показывает, что было бы исправлено, без изменений. Ошибки не роняют прогон — собираются в отчёт.
